...
smooth_curves <- list()
predictions <- list()
lambda_mat <- as.data.frame(matrix(0, ncol = 6, nrow = length(treatments) * M))
...
for (i in 1:length(treatments)){
  ...
  outcome <- c("kg", "kd", "kr")
  if (var(dat_Y[ids, "kr"]) > 0 & length(unique(dat_Y[ids, "kr"])) > 2) {
  ...
  lambda_i <- as.data.frame(matrix(NA, ncol = 5, nrow = M))
  ...
  for (j in 1:M) {
    #print progress
    cat('\r', "Progression Monte Carlo at", round(j/M*100, 2))
    
    set.seed(123 + j)
    
    #10-fold CV mvnorm
    glm_cv <- cv.glmnet(X, Y, family = "mgaussian", alpha = 1, nfolds = 10, standardize.response = T)
  ...
  smooth_curve[, 2:3] <- smooth_curve[, 2:3]/M
  lambda_mat[(i - 1) * M + 1:M, ] <- cbind(lambda_i, treatments[i])
  
  #bring predictions back to actual size
  cv_pred[, 1:2, ] <-  exp(cv_pred[, 1:2, ])
  colnames(cv_pred) <- outcome
  
  plot_cv[[i]] <- as.data.frame(smooth_curve) %>% 
    ggplot(aes(x = log(lambda), y = cverror)) +
    geom_errorbar(aes(ymin=cverror - cverror_sd, ymax = cverror + cverror_sd), colour = "grey60") +
    geom_point(colour = "red") +
    labs(title = paste( "Smooth CV curve", treatments[i])) +
    theme_bw()
...
lambda_mat <- data.frame(lambda_mat, treatment = rep(treatments, each = M))
colnames(lambda_mat)[1:5] <- c("lambda_min", "ind_lambda_min", "lambda_1se", "ind_lambda_1se", "replication")
names(predictions) <- treatments
names(smooth_curves) <- treatments


save(lambda_mat, file = paste0("results/MVLasso_predictions/lambda_repeatedCV_", omic, ".Rdata"))
save(predictions, smooth_curves, file = paste0("results/MVLasso_predictions/predictions_repeatedCV_", omic, ".Rdata"))


pdf(file = paste0("results/figures/MVLasso_smooth_cv_curves_", omic, ".pdf"))
for (i in i:length(i)) {
  print(plot_cv[[i]])
}
...
#Retrieve intercept model
###Choose the omic
omic <- "cn"
load(paste0("results/predicted_curves/predicted_parameters_MVlasso_", omic, ".Rdata"))
...
save(sABC_treat_total, file = "results/MVLasso_predictions/sABC_over_time.Rdata")
